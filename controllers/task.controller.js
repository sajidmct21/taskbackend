...
// Get Importanta Task
export const getAllImportantTasksByUser = asyncHandler(
  async (req, res, next) => {
    const { id } = req.headers;
    if (!id) {
      throw new ApiError(400, "User id is required");
    ...
    res
      .status(200)
      .json(new ApiResponse(200, "Important Task data", importantTasksData));
  }
);

// Get All complete tasks
export const getAllCompleteTasksByUser = asyncHandler(
  async (req, res, next) => {
    const { id } = req.headers;
    if (!id) {
      throw new ApiError(400, "User id is required");
    ...
    res
      .status(200)
      .json(new ApiResponse(200, "Completed Task data", completeTasksData));
  }
);

// Get All incompleted Tasks
export const getAllIncompleteTasksByUser = asyncHandler(
  async (req, res, next) => {
    const { id } = req.headers;
    if (!id) {
      throw new ApiError(400, "User id is required");
    ...
    res
      .status(200)
      .json(
        new ApiResponse(200, "Important Incomplete Tasks", incompleteTasksData)
      );
  }
);
...
export const updateImportantTask = asyncHandler(async (req, res, next) => {
  ...
  res.status(200).json(new ApiResponse(200, "Important Task is updated",re.data.important));
});
